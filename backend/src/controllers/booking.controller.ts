...
export const createBookingHandler = (service = new BookingService()) =>
  asyncHandler(async (req: Request, res: Response) => {
  const result = await service.create(req.body, req.user);
  return res.status(201).json(ok(result));
  });

export const cancelBookingHandler = (service = new BookingService()) =>
  asyncHandler(async (req: Request, res: Response) => {
  const result = await service.cancel(req.params.id, req.user);
  return res.json(ok(result));
  });

export const listBookingsHandler = (service = new BookingService()) =>
  asyncHandler(async (req: Request, res: Response) => {
